...
class SOM:
    ...
    def rand_init_from_data(self, dataset, seed=None):
        ...
        for idx in np.ndindex(self.map_dim["w"], self.map_dim["h"]):
            self.map[idx] = dataset[int(random_generator.rand() * (len(dataset) - 1))]
            self.map[idx] /= np.linalg.norm(self.map[idx])

    def activate(self, data_item):
        """
        :param data_item: a sample from the data (length = map_dim[2])
        """
        activations = np.empty(shape=(self.map_dim["w"], self.map_dim["h"]), dtype=np.float32)
        it = np.nditer(activations, flags=['multi_index'])
        while not it.finished:
            activations[it.multi_index] = self.metrics(data_item, self.map[it.multi_index[0], it.multi_index[1]])
            it.iternext()
        return activations

    def train(self, dataset, epochs=5, seed=None):
        """
        ...
        for epoch in range(epochs):
            for idx in random_generator.permutation(len(dataset)):
                lr = lr * self.decay
    ...
    def min_activation_idx(self, data_item):
        activations = self.activate(data_item)
